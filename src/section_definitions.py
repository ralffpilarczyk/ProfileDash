...
# --- Section 26: Buyside Due Diligence - Macro (Simplified) ---
    {
        ...
        "specs": "From buyside view, analyze top 3 macro risk trends (next 12-24m). Focus on downside (slowing growth, rates, inflation, labor, supply chain, FX, geopolitical). For each: Name/Category, Risk Description, Company Sensitivity Analysis (quantify negative impact scenarios), Competitive Benchmarking (relative vulnerability), Mitigation Assessment (effectiveness/limitations), Formulate 2-3 Due Diligence questions. Provide detailed analysis in 'analysis_text'. **Within the analysis_text, also: deepen the analysis of sensitivity/vulnerability (linking to Section 1 structure); rigorously evaluate mitigation vs peers (Section 5); highlight key macro risks requiring deep diligence; ensure Due Diligence questions are highly specific, probing, and data-driven.** List key risks in the structured field.",
        "schema": add_standard_fields({
            "type": "object",
            "properties": {
...
# --- Section 27: Buyside Due Diligence - Industry (Simplified) ---
    {
        ...
        "specs": "From buyside view, analyze top 3 industry risk trends (next 12-24m). Focus on downside (disruptive tech, competition, customer shifts, industry regulation, biz model obsolescence). For each: Name/Category, Risk Description/Why risk to Company, Company Vulnerability Analysis (quantify exposure/gap/scenario impact), Competitive Benchmarking (positioning/response vs peers), Company Response Assessment (adequacy/limitations), Formulate 2-3 Due Diligence questions. Provide detailed analysis in 'analysis_text'. **Within the analysis_text, also: deepen analysis of vulnerability (linking to business model, Section 2 Products, Section 13 Strategy); critically evaluate realism/sufficiency of strategic response vs industry pace and Competitors (Section 5); probe for potentially unacknowledged industry risks; synthesize the most critical industry risks for deep diligence; ensure Due Diligence questions are highly specific, challenging, and evidence-seeking.** List key risks in the structured field.",
        "schema": add_standard_fields({
            "type": "object",
            "properties": {
                 "identified_risks_list": {
        ...
        "template": {
            ...
            "analysis_text": "Industry risks require diligence [ref1]. 1) Software Disruption: Risk from software-natives (e.g., SoftwareAutomation Inc) challenging traditional models [ref2]. Company vulnerable due to lagging capabilities (win rate vs natives 42% down from 52% [ref3], cloud platform score 6.2/10 [ref4]), risking recurring revenue goals (potential miss $85-140M by 2025 [ref3]). Competitor A investing more ($350M/yr vs $150M/yr est.) [ref4]. Company response (partnerships, dev centers) lacks scale [ref5]. Due Diligence Qs focus on platform roadmap and digital deal churn. 2) Hardware Commoditization: Accelerating price erosion (-3.2% Q1'24 [ref8]) in standard hardware (~42% revenue) driven by Asian competitors [ref7]. Risks margin compression (potential -1.5 ppts EBITDA impact scenario [ref8]). Company more exposed than Competitor A (42% vs 35% revenue [ref9]). Mitigation (bundling, cost reduction) partially effective [ref10]. Due Diligence Qs focus on SKU-level margins and pricing response plans. 3) Regulatory Hurdles...",
            "footnotes": [], # Fixed placeholder
            "notes": None
        }
    },
# --- Section 28: Buyside Due Diligence - Competitive Positioning (Simplified) ---
    {
        ...
        "specs": "From buyside view, critically evaluate competitive positioning. Analyze market share trends (overall, segment, geo; last 24-36m; rate of change vs competitors). Assess defensibility/sustainability of product differentiation (use objective metrics, validate claims, identify erosion). Evaluate actual pricing power (realization, discounting, premium/discount vs competitors, cost pass-through). Analyze relative cost position (benchmark COGS%, SG&A%, Mfg cost/unit, R&D efficiency vs peers). Critically assess claimed advantages (validate magnitude, sustainability, economic contribution). Identify key disadvantages/vulnerabilities. Formulate 2-3 Due Diligence questions per topic area. Provide detailed analysis in 'analysis_text'. **Within the analysis_text, also: scrutinize market share quality/drivers and challenge market definitions; actively test differentiation claims for erosion vs Competitors (Section 5); probe pricing power limits/sustainability; identify cost structure vulnerabilities (linking to Section 4) and assess R&D effectiveness/ROI; rigorously validate advantages while amplifying Weaknesses (Section 16) and Constraints (Section 14) as exploitable disadvantages; synthesize the net competitive position and highlight critical competitive risks; ensure Due Diligence questions are highly specific, challenging, and data-requesting based on this analysis.** List key risks in the structured field (renaming from `key_dd_questions` for clarity, though questions generated in text).",
        "schema": add_standard_fields({
                        "type": "object",
                        "properties": {
                 "positioning_summary": {
...
# --- Section 30: Buyside Due Diligence - Financial Performance (Simplified) ---
    {
        ...
        "template": {
             ...
             "analysis_text": "Financial diligence areas [ref1]. 1) Recurring Revenue Quality: Growing (22%->28.5% mix Q1'22-Q1'24 [ref2]) but lags leader A (35%) [ref3]. Quality seems high (60% subscription) [ref4], but sustainability depends on closing software gap [ref5]. Due Diligence Qs probe cohort retention/expansion and contract lengths. 2) Gross Margin Sustainability: Declined from 43.8% (Q1'22) to 41.3% (Q1'24), now below industry avg (43.1%) [ref31/ref32]. High risk of further compression from hardware price erosion (-3.2% Q1'24 [ref8]) and input costs [ref34]. Due Diligence Qs focus on margin bridge and cost pass-through. 3) FCF Conversion: Strong (87.5% OCF/EBITDA FY23) but aided by lower capex recently [ref5 from Sec 23]. Sustainability requires maintaining WC improvements [ref5 from Sec 23]. Due Diligence Qs probe WC details. 4) Capitalized Software Costs: Need review of policy, amortization, potential impairment risk given tech shifts. Due Diligence Qs request policy & testing details. 5) Accounts Receivable: Check quality via aging/allowance adequacy. Due Diligence Qs ask for aging/allowance methodology.",
             "footnotes": [], # Fixed placeholder
             "notes": None
        }
    },
